...
import { FiEdit2, FiTrash2, FiCalendar, FiCheckSquare } from 'react-icons/fi';
import { FaFire } from 'react-icons/fa';
import { useScholarshipContext } from '../context/scholarshipContext.jsx'; 

function Scholarships({ handleAddButton, handleEdit, handleDelete }) { 
  const {
            ...
            <div key={scholarship.id} className='scholarship-card'>
              ...
              <div className='metadata-grid'>
                ...
                <div>
                  ...
                  <span className={`status-badge status-${scholarship.status .toLowerCase() .replace(/\s+/g, '-')}`} >
                    {scholarship.status}
                  </span>
                </div>
                ...
                <div className="task-list-container">
                  <div className="task-section-header">
                    ...
                    <span>
                      {((scholarship.tasks || []).filter(t => t.completed).length)} / {((scholarship.tasks || []).length || 3)}
                    </span>
                  </div>
                  {(scholarship.tasks || [
